...
fn main() {
    ...
    let conn = create_new_conn(curr_path_str);
    let ttype = Type::new("rtag_type").unwrap();
    ...
    match matches.subcommand() {
        ("tag", Some(clone_matches)) => {
            conn.tag_path_or_http(
                Some(&ttype),
                String::from(clone_matches.value_of("path").unwrap()),
                String::from(clone_matches.value_of("tag").unwrap()),
            ).unwrap();
        ...
        ("create", Some(create_tag_matches)) => {
            conn.create_tag(&ttype,
                &String::from(create_tag_matches.value_of("tag").unwrap())).unwrap();

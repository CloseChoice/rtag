mod struct_tests {
    ...
    fn test_create_tag_struct_with_files() {
        ...
        let result = Tag::new(t1, &gconn);

        let expected = Tag {
            id: t1,
            name: String::from("tag1"),
            items: [Item{
                id: Uuid::from_u128(1 as u128),
                itemtype: String::from("path"),
                name: String::from("tests/fixtures/paper1"),
                tags: Vec::new(),
            },
            Item {
                id: Uuid::from_u128(2 as u128),
                itemtype: String::from("path"),
                name: String::from("tests/fixtures/paper2"),
                tags: Vec::new(),
            }].to_vec()
        };

        println!("this is the name before assert {}", result.name);
        // assert_eq!(result.name, expected.name);
        assert_eq!(result.id, expected.id);
        assert_eq!(result.items[0].name, expected.items[0].name);
        assert_eq!(result.items[1].name, expected.items[1].name);
        assert!(result.items[0].tags.is_empty());
        assert!(result.items[1].tags.is_empty());
    }
